...
const CartPage = () => {
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get("https://ecommerce-backend-enap.onrender.com/api/v1/product/braintree/token");
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post("https://ecommerce-backend-enap.onrender.com/api/v1/product/braintree/payment", {
        nonce,
        cart,
      });
      setLoading(false);
